...
class Container(pygame.sprite.Sprite):
    ...
    def set_image(self, image, width=None, height=None, ratio=1):
        if width and height:
            ...
            self.image = pygame.transform.smoothscale(image, (width, int(image.get_height()*width/image.get_width())))
        elif height:
            self.image = pygame.transform.smoothscale(image, (int(image.get_width()*height/image.get_height()), height))
        else:
